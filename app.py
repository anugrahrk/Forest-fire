...
# Load the models
model1 = joblib.load("model/classification.joblib")
model2 = joblib.load("model/regression.joblib")
...
# MAIL_FROM = os.getenv('MAIL_FROM')  # Must be a verified domain in MailerSend
...
def alert():
    return render_template('alert.html', MAPBOX_TOKEN=os.getenv('MAPBOX_TOKEN'))
...
state_dict = torch.load("model/wildfire_satellite_detection_model.pth")  # Load state dict
...
model4.load_state_dict(torch.load('model/camera_detection.pt', map_location=device))
...
#alert
# Email Configuration
# def send_email(latitude,longitude,to_email, probability, status):
#     subject = "Satellite Fire/Smoke Detection Result"
#     body = f"""
#     Fire/Smoke Detection Results:
#     Confidence: {probability}%
#     Status: {'Fire/Smoke Detected' if status == 1 else 'No Fire/Smoke Detected'}
